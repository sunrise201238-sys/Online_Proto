...
|---|---|---|---|---|
| AA12 | 4.8 blasts/s (208 ms) | 24 (8×3, point-blank) | **115.4** | ~69.0 (20-drum + 3 s reload) |
...
**Handling (stun + spread bloom):**

| | Stun | Base SA | Bloom / shot | SA cap | Recovery | Sure-hit (base → cap) |
|---|---|---|---|---|---|---|
| M4 | 100 ms @ 0.25 | 0.02 | +0.002 | 0.06 | 0.035/s, 200 ms after the last shot | 160 → 53 |
| FAMAS | 100 ms @ 0.25 | 0.02 | +0.003 | 0.06 | 0.035/s, 200 ms | 160 → 53 |
| evo3 | 100 ms @ 0.25 | 0.03 | +0.003 | 0.08 | 0.05/s, 200 ms | 107 → 40 |
| P90 | 100 ms @ 0.25 | 0.03 | +0.003 | 0.08 | 0.05/s, 200 ms | 107 → 40 |
| AA12 | 100 ms @ 0.25 | pattern (see below) | — | — | — | pattern |
| M1014 | 100 ms @ 0.25 | pattern, 1.4× wide (see below) | — | — | — | pattern |
| RPK | 100 ms @ 0.25 | 0.02 | +0.004 | 0.12 | 0.03/s, 200 ms | 160 → 27 |
| NEGEV | 100 ms @ 0.25 | 0.02 | +0.003 | 0.12 | 0.03/s, 200 ms | 160 → 27 |
| M14 | 100 ms @ 0.25 | 0.02 | +0.10 | 0.20 | 0.17/s, no delay | 160 → 16 |
| SVD | 100 ms @ 0.25 | 0.02 | +0.10 | 0.40 | 0.17/s, no delay | 160 → 8 |
| PSG1 | 100 ms @ 0.25 | 0.02 | — | — | — | 160 |
| Railgun | 100 ms @ 0.25 | — (beam) | — | — | — | instant |

P90 is the FAMAS's cadence in an SMG chassis: the 80 ms tick slot (12.5 shots/s) at 3.5 damage on a 50-round mag behind a 1.5 s reload — 3.9 s of continuous fire (175 damage per mag), the longest of the four ARs and SMGs by some way (M4 2.8 s, FAMAS 1.9 s, evo3 1.9 s). She wears the AR spread profile (base SA 0.02, blooming +0.003 per shot to a 0.08 cap — the same accuracy family as the M4 and FAMAS) and, since 2026-08-10, the AR stun as well: a chip gun that gives up per-shot weight for staying on the trigger twice as long, but whose hits now actually hold a target in place while it does.

**Reload and stun (2026-08-10, was 2 s / 50 ms @ 0.50).** The 50-round magazine is the point of the gun and the longest reload in its class was cancelling it out; 1.5 s brings sustained damage from 29.6 to 32.3. The stun move matters more. P90 and M4 were twins on everything a damage model can see — both 3,150 damage per minute at the time (M4 has since dropped to 4 damage, 2,800/min, 2026-09-21), same base spread cone, same projectile speed, same walk and sprint — and the *only* live difference between them was the stun profile, which is where the 5.7×28 round's real signature (penetration) has to live in a game with no armour system. Against bots, isolating the stun swap alone is worth +9.5 and the reload alone +4.9; together P90 lands at **44.4% (was 28.4%)** in 1v1 — out of last-but-one and level with the M4. Measured over 28,080 matches before and 9,504 after, all twelve units, all nine maps, both spawn orders, fight-to-KO (±1.4 / ±2.4).
...
**Simulated hit rates** (Monte-Carlo of the range protocol; every weapon fires from **50 units**; 200,000 shots per cell — shotgun rows show per-pellet rates; the evo3 and P90 rows are at their pre-2026-09-22 base cones of 0.04 and 0.02 — both are 0.03 now, so read evo3 as slightly better and P90 as slightly worse than shown against movers):
...
- **Both hold to fire** (2026-08-09 — M1014 was tap-only until then). They split on how the ammo comes back, not on the trigger: AA12 empties a **20-shell drum** at ~4.8 blasts/s and buys it back in one 3 s stroke (4 s until 2026-09-22), while M1014 runs a **7-shell magazine on per-shell regen** — never a famine, never a burst either. AA12 is the sustained-pressure half of the pair, walking at the MGs' 12 to pay for it.
...
- **M14** — 10 damage on a 20-round mag, base SA 0.02 (sure-hit ~160). Bloom is the whole story of the gun: each shot adds **0.10**, capped at 0.20, recovering 0.17/s from the moment of the shot — the first round is pin-point, a follow-up 336 ms later leaves at 0.063 (sure-hit ~51), the third at 0.106, and a held trigger settles near 0.143 (sure-hit ~22). Ten shots to kill leaves ten in reserve, and at the base cone it is the best walker-tracker of any ordinary bullet (82% at 50 units).
- **SVD** — 12 damage on a 10-round mag, base SA 0.02 (was 0.04; sure-hit ~160 on a fresh trigger). The same +0.10 per shot and 0.17/s recovery as the M14, but the cap is 0.40, so a held trigger never settles: 0.063, 0.106, 0.149 … up to ~0.34 by the ninth round (sure-hit ~9). Nine shots to kill leaves exactly **one** spare — able to finish a full-HP target on one magazine, where the retired Laser's 8-bolt mag capped at 96 and always fell four short. The punch is paid for in spray discipline rather than reach.
...
**Bloom gate and suppressing fire** (2026-09-21; gate line retuned 2026-09-22) — a bot fires freely only while its target sits inside the gun's *current* **gate line**, re-checked every tick from the live distance: one closed-form division, run before the obstacle scan so an out-of-range poll costs nothing. The line is a fixed width divided by the bloomed cone: for the **auto weapons it is 8.84 ÷ cone — the distance where the cone still lands one round in three on a standing target** (the 3.2 × 6.4 capsule is a third of the cone's disc at radius 4.42; 2.76× the sure-hit line); for the **marksman rifles it is the sure-hit line itself, 3.2 ÷ cone** (`botGateWidth: 3.2`). Owner decision after the duel measurements below: "the old mechanic remains as the bottom logic, 33% for the autos, 100% for the semi rifles". At their caps the autos' lines sit at 147 (M4, FAMAS), 110 (evo3, P90) and 74 (RPK, NEGEV), so inside the rifles' and SMGs' 43–63 bands the gate never trips and what you see there is the plain **burst / rest rhythm** of the trigger discipline above; the machine guns at their 80-unit band spray 23 (RPK) or 30 (NEGEV) rounds before the line closes on the target, then **hold until the cone has fully recovered** (200 ms + 0.09 ÷ 0.03 ≈ 3.3 s) — the line drifting back out past a standing target does not release the hold, so there is no one-round trickle — and open up again from the base cone. A target that **closes in** past where the line stood when the hold began releases it at once. Beyond the base cone's line (442 for the 0.02 guns, 295 for the 0.03 SMGs — farther than any arena) a recovered auto would fire a committed **suppress burst** (five rounds, ten for the SMGs) and repeat. The **marksman rifles** (M14, SVD) live on the gate: each shot adds 0.10 of bloom and 0.17/s takes 0.59 s to shed it, so at their band they settle at one shot per ~0.6 s; their suppress burst is a single round (`botSuppressBurst: 1`). Shotguns (fixed pattern) and the snipers (their cone out-reaches lock range) are never gated. Offline and online run the same rule (shared `bloom.js`, `botMayFire` / `botGateDistance`). A burst on this rhythm is the whole magazine for every auto (`botFireCap` equals the mag), so for the machine guns the reload is the rest.

**Bot fire-gate thresholds** — the hit rate on a standing target below which the bot stops firing (the line is *width ÷ current cone*; 33% = width 8.84, 100% = the 3.2 sure-hit width). One setting for both modes — this line has no per-mode override (the BA line ungates its machine guns, 2026-09-25; the demo line keeps them on the 33% line):

| Unit | 1v1 | 2v2 |
|---|---|---|
| M4, FAMAS, evo3, P90 | 33% | 33% |
| RPK, NEGEV | 33% | 33% |
| M14, SVD | 100% | 100% |
| M1014, AA12, PSG1, Railgun | never gated | never gated |

*Why the line moved (measured 2026-09-22, bot duels, 12 units round-robin on Plain Field / Streets / Factory / Square, 96 games per pairing, 6,336 per setting, at the day's final data — shotgun pellets 350, marksman reload 2 s):* with the gate on the sure-hit line the marksman rifles won 78–81% and every auto lost to them 3–12 games in 96, because the gate held fire whenever the cone could not guarantee a hit on a *standing* target while the actual target was a sprinting bot that every cone missed four times in five — the gate threw away the volume that was the only compensation. Traces showed the autos landing 17–26% of their rounds either way, the marksman rifles 45–55%. The 33% line gives back the volume — sure-hit line → 33% line: NEGEV 48 → 74%, RPK 42 → 69%, M14 81 → 65%, SVD 78 → 62%, P90 39 → 59%, AA12 71 → 57%, evo3 36 → 50%, PSG1 52 → 45%, M1014 53 → 35%, FAMAS 34 → 30%, M4 39 → 29%, Railgun 27 → 24%. (An earlier run of this table carried a hidden 7-shell shotgun in the AA12 slot; these figures use the real AA12.) The same batch at the old 300 pellet speed puts AA12 at 48% and M1014 at 25%, everything else within four points — the 350 pellets are worth about ten points to the shotguns. The small-magazine rifles and the Railgun sit at the bottom of the volume race that follows; that is the next tuning front, outside the gate.
