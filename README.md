...
**Orders** — three kinds, layered on top of the bot rather than replacing it:
...
- **Hide / Fight** (2026-09-26) — **touch-and-hold** your unit (marker or card) for 0.4 s: a gun (Fight) pops at the upper-left and a shield (Hide) at the upper-right (flipped inward at the screen edge); slide onto one and **release on it** to set the stance (release anywhere else cancels). **Hide** wipes the unit's move and lock orders and refuses new ones (a red *Hiding — double-tap to release* note) until Fight or a double-tap clears it. A hiding unit's first priority is line of sight: every tick it checks whether **both live enemies** can see it; if they can, it walks the shortest route (a walk-only Dijkstra over the nav grid, no distance cap, capped at 600 cells per search, at most one search per 0.5 s per unit and one per tick per match) to the nearest cell hidden from both — or, when no such cell exists, from the **nearest** enemy — and then stands still there, never peeking. Standing still makes its return fire pin-point (the bloom stillness rule), and it **does fire back** whenever a shot exists from where it stands. Being hit does **not** trigger Defense while hiding (hit-stun still applies; the anti-glint dodge and the sniper charge rules are untouched); the cover reload is superseded. Sprints on the way are latched: a dash arms only above **50 boost** and may spend down to the **8-boost** hard floor. Fight is plain autonomy — no badge. Online the order travels as `order:stance` (same per-unit 0.5 s rate limit; a move/lock refused for hiding is acked with `hidden`), lives in the team-scoped command side-table like the other two, and never reaches the enemy.
...
**Status icons** — gold, on both the unit's marker and its info card: **`!`** while a move order stands (en route plus the 20 s anchor), an **eye** while force-locking (pairing with the colored triangles on the enemy); both side by side when both are active, a **shield** while hiding (which by construction has neither), no icon when the unit is autonomous. On the info card the icons are 14×12 px (11×9.5 compact) since 2026-09-26 — the 20×17 originals hung below the card frame once the portrait grew the name row.
...
**Measured hit rates** (2026-09-26 — remeasured for the spread bloom; the pre-bloom Shooting Range lanes are retired). Once a cone grows with every round, one percentage no longer describes a gun: what matters is **how the hit chance decays across a magazine while the shooter is on the move**. So the new protocol is a walking full-magazine spray, run on the shared sim (the code the server runs) on Plain Field, 200 runs per series:

- **Shooter:** strafes sideways at her own walk speed (16 or 12 u/s; Koyuki 10, Hina 8), reversing every 400 ms so she is never still and every round blooms, and holds the trigger through **one whole magazine** (tap-fired for the shotguns and snipers; a charging sniper is rooted by the sim, as in play), **obeying the bot fire gate** (`botMayFire`): only the marksman rifles' line — the 100% sure-hit line — ever closes inside their own lock range, so Fubuki and Aris fire one round per 0.6 s from a fully recovered cone, exactly as their bots do; every other gun's line sits beyond its lock range even at the cap, and the gate never trips. Every unit fires from **her own lock range** only.
- **Target:** a real unit — the 3.2-wide hit capsule, not the Range's signs — ping-ponging on a ±35° arc at lock range around the shooter, so it always moves across the line of fire, at three speeds: standing, the shooter's own walk speed, and that walk speed + 11.76 (the sprint boost). Reversal at the arc ends is instant, so the speed never dips. Shots aim at the target's current position, exactly as the game does (no lead).
- **Scoring:** every round is a hit or a miss (shotguns: pellets landed ÷ 8 per blast; Kei's beam counts as a round). Each curve is the hit chance of the *n*-th round of the magazine, averaged over the 200 runs; the numbers in the table are the mean over the whole magazine and, in brackets, the mean of its first third → last third.

![Per-round hit rate — walking full-magazine spray](docs/img/hitcurve/hit_curves.png)

| @ own lock · magazine | Still target | Walking target | Sprinting target |
|---|---|---|---|
| Saori @56 · 30 rds | 100% (100→99) | 81% (93→73) | 35% (28→39) |
| Asuna @56 · 25 rds | 100% (100→99) | 80% (92→73) | 34% (27→40) |
| Atsuko @50 · 30 rds | 95% (100→90) | 65% (71→60) | 29% (21→34) |
| Marina @50 · 71 rds | 73% (82→69) | 59% (60→58) | 36% (34→38) |
| Hoshino @40 · 7 rds | 83% (82→83) | 46% (45→47) | 17% (16→17) |
| Haruka @40 · 7 rds | 59% (58→59) | 47% (46→47) | 25% (25→25) |
| Koyuki @80 · 100 rds | 49% (65→41) | 44% (52→40) | 29% (23→32) |
| Hina @80 · 250 rds | 38% (45→34) | 37% (41→34) | 31% (31→31) |
| Fubuki @56 · 30 rds | 100% (100→100) | 100% (100→100) | 19% (18→20) |
| Aris @56 · 8 rds | 100% (100→100) | 100% (100→100) | 62% (60→62) |
| Aru @120 · 5 rds | 100% (100→100) | 100% (100→100) | 89% (90→93) |
| Kei @120 · 5 rds | 100% (100→100) | 100% (100→100) | 100% (100→100) |

*Reading it:* the first→last gap is the bloom. The 900 RPM rifles open a magazine at 100% on a walking target and close it in the low 70s; Atsuko slips 100→90 even on a standing target: her capped cone's sure-hit line is 40 u, inside her 50 u lock, and a 30-round spray gets there by round 20; Koyuki's 100-round belt and Hina's 250 reach their capped cone inside the first third and sit there (~40% and ~34%), which is why their bots fire ungated. A flat pair means the gun never leaves its resting cone: the shotguns (7 blasts, a fixed pattern), and Hina and Koyuki from round ~40 on. The marksman rifles are flat for the opposite reason: on their 100% gate they never fire bloomed, so every round is a 0.02-cone shot — 100% on anything walking, and against a sprinter only the flight-time drift is left: Fubuki 19%, Aris 62%, her laser bolt hitting with a 0.4 wider radius and its whole 64-long trailing body, which forgives more of the drift than Fubuki's point bullet. (Held down through the magazine instead, ignoring the gate, they spray at their caps — Fubuki 35 / 33 / 28%, Aris 71 / 74 / 56% — which is what the gate is for.) The walk column is no longer one target speed for everyone — the 16 u/s SMG targets are harder than the 12 u/s rifle targets — so compare it as "each unit against her own peer". Sprint is no longer untouchable: crossing the line of fire at 24–28 u/s still gets clipped a quarter to a third of the time, and the assault rifles' sprint curve *rises* through the magazine (28→39) as the widening cone starts to cover the flight-time lead. The snipers are the flat lines at the top: Aru's 2,500 u/s bolt crosses 120 u in under 50 ms, so even a sprinter drifts about one unit before impact, and Kei's beam is instant.
