...
| evo3 | 50 ms @ 0.50 | 0.04 | +0.003 | 0.09 | 0.05/s, 200 ms | 80 → 36 |
...
| RPK | 100 ms @ 0.25 | 0.02 | +0.004 | 0.12 | 0.05/s, 200 ms | 160 → 27 |
| NEGEV | 50 ms @ 0.85 | 0.02 | +0.003 | 0.12 | 0.05/s, 200 ms | 160 → 27 |
...
**Bloom gate and suppressing fire** (2026-09-21) — a bot fires freely only while its target sits inside the gun's *current* sure-hit distance (3.2 ÷ the bloomed cone), re-checked every tick from the live distance: one closed-form division, run before the obstacle scan so an out-of-range poll costs nothing. Once the cone has bloomed past the target's distance an **auto weapon holds until the cone has fully recovered** — the line drifting back out past a standing target does not release it, so there is no one-round trickle — then opens up again: the burst runs at the mechanical rate until the cone is past the target once more, so the closer the target the longer the burst (an RPK at its 80-unit band: six rounds, a 0.7 s recovery, six rounds …; an M4 at 56: nineteen rounds, then 1.3 s). A target that **closes in** past where the line stood when the hold began releases it at once. If the target is beyond the base cone's sure-hit distance (evo3 past 80, the other autos past 160), a recovered bot fires a committed **five-round suppress burst** instead, then recovers and repeats. **Marksman rifles** (M14, SVD) take no hold: they re-fire the moment the cone is back under the line, and suppress with a single round — at their band that settles at one shot per ~0.6 s either way, because each shot adds 0.10 of bloom and 0.17/s takes 0.59 s to shed it. Shotguns (fixed pattern) and the snipers (their cone out-reaches lock range) are never gated. Offline and online run the same rule (shared `bloom.js`, `botMayFire`).
...
- **Lock bracket bloom** (2026-09-21): the crosshair brackets around your target are drawn at half their former base size (owner pick from in-game samples) and grow with your *own* current spread bloom on a universal scale — ×1 at the base cone, ×2.25 at the gun's own cap — so every weapon's bracket reads "how much of my bloom budget is spent", never the absolute cone size. The commander's lock-share triangles and the command-mode markers keep their size.
