...
Reading the DPS table: the shotgun rows are the most theoretical — all 8 pellets only land point-blank. M1014's 7-shell magazine burns in ~1.7 s before per-shell regen throttles the long run; AA12 empties her 20-drum in ~4 s of auto fire and then pays one full 4 s reload. The sniper rows use cycle math (cooldown + floor charge) at full range-tier damage. The tight 42–55 spread across the five mid-table autos (evo3, FAMAS, M4, RPK, P90) is deliberate — fights are decided by accuracy curves, uptime, and positioning rather than raw DPS. The two marksman rifles sit below that band on burst but hold ~24 sustained, close to the autos: their damage arrives in fewer, larger pieces rather than more slowly.
...
**Bloom gate and suppressing fire** (2026-09-21) — a bot fires freely only while its target sits inside the gun's *current* sure-hit distance (3.2 ÷ the bloomed cone), re-checked every tick from the live distance: one closed-form division, run before the obstacle scan so an out-of-range poll costs nothing. Once the cone has bloomed past the target's distance an **auto weapon holds until the cone has fully recovered** — the line drifting back out past a standing target does not release it, so there is no one-round trickle — then opens up again: the burst runs at the mechanical rate until the cone is past the target once more, so the closer the target the longer the burst (an RPK at its 80-unit band: six rounds, a 1.0 s recovery, six rounds …; an M4 at 56: nineteen rounds, then 1.3 s). A target that **closes in** past where the line stood when the hold began releases it at once. If the target is beyond the base cone's sure-hit distance (evo3 and P90 past 107 since their 0.03 base of 2026-09-22, the other autos past 160), a recovered bot fires a committed **suppress burst** instead, then recovers and repeats: five rounds for the rifles and machine guns, **ten for the SMGs** (evo3 and P90, owner call 2026-09-22 — the SMGs' 64/80 ms cadence and quick 0.05/s recovery keep the cycle short, so the longer burst lands more rounds per recovery). The same hold applies to the **marksman rifles** (M14, SVD): each shot adds 0.10 of bloom and 0.17/s takes 0.59 s to shed it, so at their band they settle at one shot per ~0.6 s; their suppress burst is a single round (`botSuppressBurst: 1`). Shotguns (fixed pattern) and the snipers (their cone out-reaches lock range) are never gated. Offline and online run the same rule (shared `bloom.js`, `botMayFire`).
