...
**Bots vs. the sniper.**
- **As the shooter:** Aru bots release at the **0.5 s floor 70%** of the time (a fast snap) and hold to the **full 1 s charge the other 30%** — no in-between releases. Kei bots use the same 70/30 split (his full charge triggers the sweep channel).
- **On defense:** when a glint aimed at it appears — from **any** enemy, locked or not (mirroring the human's edge-indicator awareness; earliest active charge wins) — the bot **rolls its reaction per charge**: **70% at 0.5 s** (the dodge's i-frames start alongside the earliest possible cancel, covering **every floor snap at any range** — but a full hold sails in after they end) or **30% at 0.8 s** (deliberately late: a floor snap lands first and cancels the pending dodge, but the i-frames ~0.8–1.1 s sit exactly on a **full hold's** impact and dodge it). One dodge (0.3 s i-frames) plus a **0.52 s** committed sprint, both perpendicular to that sniper's line of fire; lock and return fire stay on the current target throughout. Shooter rolls snap/hold at 70/30, defender rolls fast/slow at 70/30 — neither side can be read, and the measured bot-vs-bot equilibrium is **~42% of charges converting** (snaps beat slow rolls, holds beat fast rolls). Mid-charge hits still cancel a pending dodge, and a cooldown- or boost-blocked defender still eats the shot.
