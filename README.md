...
- **Profile cards** carry seven live-read stats (Mag / Dmg / RPM / Weight / Reload / Stun / Spread). Tiered damages show all tiers (PSG1 `50/35/20`, Railgun `30/20`, shotguns per-pellet `5 ×8`); stun uses tier words (Heavy/Normal/Light); **Weight** (2026-09-22, replacing the projectile-speed row Slow/Normal/Fast/Instant) is the unit's walk-speed tier — **Light** (walk 16: evo3, P90, M1014), **Medium** (14: M4, FAMAS, M14, SVD, PSG1, Railgun), **Heavy** (12: AA12, RPK, NEGEV); and Spread is a rough read of how wide the gun gets when sprayed, keyed on its bloom cap: **None** (never blooms — the snipers, the Railgun beam and the fixed-pattern shotguns), **Low** (cap ≤ 0.06: M4, FAMAS), **Medium** (≤ 0.12: evo3, P90, RPK, NEGEV), **High** (M14, SVD). The seventh cell, a **Spread scatter simulation** (a standing target's width at range 60 with the engine's real fire math at the base cone), is **hidden since 2026-09-21** — with per-shot bloom the base-cone picture no longer tells the story; the code stays behind the `PROFILE_SPREAD_CELL` switch in `client/src/main.js`.
