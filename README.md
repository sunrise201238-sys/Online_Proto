...
- **Lock bracket bloom** (2026-09-21): the crosshair brackets around your target are drawn at half their former base size (owner pick from in-game samples) and grow with your *own* current spread bloom on **one universal scale** — ×1 with no bloom, growing at the same rate on every weapon up to ×4.5 at 0.38 of bloom, the SVD's full cap. The same amount of spread therefore reads the same size whichever gun you hold: an M4 at its 0.06 cap shows about ×1.37, an RPK at 0.12 about ×1.9, the M14 about ×2.7, the SVD the full ×4.5. The commander's lock-share triangles and the command-mode markers keep their size.
