...
- **Profile cards** carry six live-read stats (Mag / Dmg / RPM / Spd / Reload / Stun). Tiered damages show all tiers (PSG1 `50/35/20`, Railgun `30/20`, shotguns per-pellet `5 ×8`); speed and stun use tier words (Slow/Normal/Fast/Instant; Heavy/Normal/Light). The seventh cell, a **Spread scatter simulation** (a standing target's width at range 60 with the engine's real fire math at the base cone), is **hidden since 2026-09-21** — with per-shot bloom the base-cone picture no longer tells the story; the code stays behind the `PROFILE_SPREAD_CELL` switch in `client/src/main.js`.
