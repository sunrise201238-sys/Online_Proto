...
- **On defense:** when a glint aimed at it appears — from **any** enemy, locked or not (mirroring the human's edge-indicator awareness; earliest active charge wins) — the bot ignores it for a fixed **0.5 s**, then does **one** dodge (0.3 s i-frames) followed by a **0.52 s** committed sprint, both perpendicular to that sniper's line of fire. Its lock and return fire stay on its current target throughout. At a 0.5 s reaction the dodge starts the same instant as the fastest possible cancel, so flight time makes **every floor snap dodgeable at any range**; what still lands is the held shot (arriving after the spent dodge's i-frames), a mid-charge hit that cancels the scheduled dodge, or a defender whose dodge is cooldown- or boost-blocked. Measured bot-vs-bot equilibrium: ~30% of charges convert, all of them holds.
