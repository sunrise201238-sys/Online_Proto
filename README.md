...
*Why the line moved (measured 2026-09-22, bot duels, 12 units round-robin on Plain Field / Streets / Factory / Square, 96 games per pairing, 6,336 per setting):* with the gate on the sure-hit line the marksman rifles won 84–85% and every auto lost to them 3–12 games in 96, because the gate held fire whenever the cone could not guarantee a hit on a *standing* target while the actual target was a sprinting bot that every cone missed four times in five — the gate threw away the volume that was the only compensation. Traces showed the autos landing 17–26% of their rounds either way, the marksman rifles 45–55%. The 33% line gives back the volume: RPK 45 → 72%, NEGEV 53 → 75%, P90 48 → 61%, evo3 45 → 57%, SVD 85 → 70%, M14 84 → 68%, PSG1 53 → 48%, FAMAS 42 → 38%, M4 45 → 37%, M1014 39 → 27%, Railgun 26 → 25%, AA12 33 → 22%. The small-magazine rifles and the shotguns lose ground in the volume race that follows; that is the next tuning front, outside the gate.
