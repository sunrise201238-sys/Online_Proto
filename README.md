...
**Bloom gate** (2026-09-21) — a bot only pulls the trigger while its target sits inside the gun's *current* sure-hit distance (3.2 ÷ the bloomed cone), re-checked every tick from the live distance: one closed-form division, run before the obstacle scan so an out-of-range poll costs nothing. Spraying therefore paces itself — the burst runs at the mechanical rate until the cone has bloomed past the target's distance, holds while the cone recovers, then resumes; the closer the target, the longer the burst (an RPK at its 80-unit band gets six full-rate rounds before it has to wait, then trickles at roughly three a second). Shotguns (fixed pattern) and the snipers (their cone out-reaches lock range) are never gated. Offline and online run the same rule.
